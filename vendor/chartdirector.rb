...
class AutoMethod
	...
	def initialize(this, parent = nil)
		@this = this
	end
	...
	def method_missing(name, *args)
		...
		if defaultArgs and defaultArgs.length > 0 and defaultArgs[0]
			return defaultArgs[0].new(ret, self)
		else
			return ret
		end
	end
...
class DrawArea < AutoMethod
	...
	def initialize(this = nil, parent = nil)
		if not this
...
class ContourLayer < Layer
	...
	def setExactContour(contour = 1, markContour = "same")
		if markContour == "same"
			markContour = contour
		end
...
class XYChart < BaseChart
	...
	def getYValue(yCoor, yAxis = nil)
		return _r("XYChart.getYValue", @this, yCoor, decodePtr(yAxis))
	end
...
class ThreeDScatterGroup < AutoMethod
	DefaultArgs = {
		"setDataSymbol4" => [nil, 4, 11, -1, -1],
		"setDropLine" => [nil, 2, LineColor, 1],
		"setLegendIcon" => [nil, 3, -1, -1]
	}
	def setDataSymbol(symbol, size = nil, fillColor = -1, edgeColor = -1, lineWidth = 1)
		if symbol.class == Array
			if not size 
		...
		_r("ThreeDScatterGroup.setDataSymbol", @this, symbol, size, fillColor, edgeColor, lineWidth)
	end
	def setDataSymbol2(image)
		if image.respond_to?("this", 1)
			setDataSymbol3(image)
		else
			_r("ThreeDScatterGroup.setDataSymbol2", @this, image)
		end
	end
	def setDataSymbol3(image)
		_r("ThreeDScatterGroup.setDataSymbol3", @this, image.this)
	end
end
...
class ThreeDScatterChart < ThreeDChart
	...
	def initialize(width, height, bgColor = BackgroundColor, edgeColor = Transparent, raisedEffect = 0)
		super(_r("ThreeDScatterChart.create", width, height, bgColor, edgeColor, raisedEffect))
	end
end
...
class RanSeries < AutoMethod
	DefaultArgs = {
		"getSeries2" => [nil, 6, -C17E308, C17E308],
		"getDateSeries" => [nil, 4, 0]
	}
	def initialize(seed)
		@this = _r("RanSeries.create", seed)
		destructor("RanSeries.destroy", @this)
	end
	def getSeries(length, minValue, maxValue, p4 = nil, p5 = -C17E308, p6 = C17E308)
		if not p4
			return _r("RanSeries.getSeries", @this, length, minValue, maxValue)
		else
			return getSeries2(length, minValue, maxValue, p4, p5, p6)
		end
	end
end
...
class WebChartViewer < AutoMethod
	...
	def setChartModel(model)
		putAttrS(":model", model)
	end
	def getChartModel()
		return getAttrS(":model")
	end

	def setFullRange(id, minValue, maxValue)
		_r("WebChartViewer.setFullRange", @this, id, minValue, maxValue)
	end
	def getValueAtViewPort(id, vpCoor, isLogScale = 0)
		return _r("WebChartViewer.getValueAtViewPort", @this, id, vpCoor, isLogScale)
	end
	def getViewPortAtValue(id, value, isLogScale = 0)
		return _r("WebChartViewer.getViewPortAtValue", @this, id, value, isLogScale)
	end
	def syncLinearAxisWithViewPort(id, axis)
		syncAxisWithViewPort(id, axis, 3)
	end
	def syncLogAxisWithViewPort(id, axis)
		syncAxisWithViewPort(id, axis, 4)
	end
	def syncDateAxisWithViewPort(id, axis)
		syncAxisWithViewPort(id, axis, 5)
	end
	def syncAxisWithViewPort(id, axis, scaleType)
		_r("WebChartViewer.syncAxisWithViewPort", @this, axis.this, id, scaleType)
		@dependencies.push(axis)
	end
	...
	def renderHTML(extraAttrs = nil)
		...
		if @request.respond_to?("fullpath", 1)
...
TTFText=>nil,
TextBox=>nil,
ThreeDScatterChart=>nil,
ThreeDScatterGroup=>nil,
TrendLayer=>nil,
VectorLayer=>nil,
XYChart=>nil,
WebChartViewer=>nil
